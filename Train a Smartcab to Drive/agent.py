...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            self.epsilon -= 0.05
            # self.epsilon = math.exp(-0.02*self.tracker) for improved
            self.tracker+=1
        return None
    ...
    def choose_action(self, state):
        ...
        if self.learning == False or random.random() <= self.epsilon:
            ...
            if self.Q[state].values().count(self.get_maxQ(state)) > 1:
                ties = True
            for key, value in self.Q[state].iteritems():
                if value == self.get_maxQ(state) and ties == False:
                    actions.insert(0,key)
                elif value == self.get_maxQ(state) and ties == True:
                    actions.insert(0,key)
...
def run():
    ...
    sim = Simulator(env, display=True, log_metrics=True, update_delay=0.01, optimized=False)
